...
const isText = (child: TemplateChildNode): child is TextNode => child.type === NodeTypes.TEXT;
...
const isInterpolation = (child: TemplateChildNode): child is InterpolationNode =>
  child.type === NodeTypes.INTERPOLATION;

export const parseAttrs = (attrs: ReadonlyArray<AttributeNode | DirectiveNode>): readonly Attr[] =>
  attrs.map(attr => {
    if (isAttr(attr)) {
...
export const parseTag = (child: PlainElementNode, children: ReadonlyArray<Tag | Text>): Tag => ({
  node: Node.Tag,
  name: child.tag,
...
export function converVueAst(ast: RootNode): readonly Nodes[] {
  const deepConvert = (children: readonly TemplateChildNode[]) =>
    children.reduce<readonly Nodes[]>((acc, child) => {
      ...
      if (isElement(child)) {
        return acc.concat(parseTag(child, deepConvert(child.children)));
      }
